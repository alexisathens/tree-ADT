...
namespace cop3530
{
    ...
    public:
        ...
        Rand(Rand &&src) //move constructor
        {
            root = src.root;
            src.root = nullptr;
        }
        
        Rand<K, V, compare, equals>& operator=(Rand& src) //copy assignment
        {
            if(this == &src)
            {
                return *this;
        ...
        Rand<K, V, compare, equals>& operator=(Rand&& src) //move assignment
        {
            if(this == &src)
            {
                return *this;
        ...
        void insert(K key, V value)
        {
            ...
            {
                int num = nrand(getSize(root) + 1); //generate random number from 0 to tree size inclusive
                if(num == 0)
                {
                    insert_root(root, key, value);
                }
        ...
        {
            if(n <= 0 || n > RAND_MAX)
                throw domain_error("Error, size out of range");
